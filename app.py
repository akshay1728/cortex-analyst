...
for idx, msg in enumerate(st.session_state.messages):
    with st.chat_message(msg["role"]):
        ...
        # Display Dynamic Chart if present via Vega-Lite
        if "figure" in msg and msg["figure"] is not None:
            ...
            try:
                ...
                # Convert Plotly data to Vega-Lite spec format
                data_list = []
                if isinstance(fig_dict, dict) and "data" in fig_dict:
                    for trace in fig_dict["data"]:
                        x_vals = trace.get("x", [])
                        y_vals = trace.get("y", [])
                        t_name = trace.get("name") or "Metric"
                        if x_vals and y_vals:
                            for x_v, y_val in zip(x_vals, y_vals):
                                data_list.append({"x": x_v, "y": y_val, "category": t_name})
                ...
                if data_list:
                    vega_spec = {
                        ...
                        "encoding": {
                            ...
                            "y": {"field": "y", "type": "quantitative", "title": "Value", "scale": {"zero": False}},
                            "color": {"field": "category", "type": "nominal"}
                        },
                        "data": {"values": data_list}
                    }
                    st.vega_lite_chart(vega_spec, use_container_width=True, key=f"hist_chart_{idx}")
                else:
                    st.dataframe(msg["data"], use_container_width=True, key=f"hist_chart_df_{idx}")
            except Exception as e_v:
                st.info(f"Chart figure available: {e_v}")
            ...
            if chart_res.should_visualize and chart_res.figure is not None:
                ...
                try:
                    ...
                    if data_list:
                        ...
                        st.vega_lite_chart(vega_spec, use_container_width=True, key=f"active_chart_{active_idx}")
                    else:
                        st.dataframe(data, use_container_width=True, key=f"active_chart_df_{active_idx}")
                except Exception as e_v:
                    logger.warning(f"Vega conversion error: {e_v}")
