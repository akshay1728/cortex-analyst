...
def collect_response(
    ...
    for evt_wrapper in events:
        ...
        # --- Extract Executed SQL Query from events/tool results ---
        possible_sql = (
            data.get("sql_query") or data.get("sql") or data.get("query") or
            (data.get("delta", {}).get("sql") if isinstance(data.get("delta"), dict) else None) or
            (data.get("tool_use", {}).get("input", {}).get("sql") if isinstance(data.get("tool_use"), dict) else None)
        )
        if possible_sql and isinstance(possible_sql, str) and possible_sql.strip().upper().startswith(("SELECT", "WITH", "SHOW", "EXPLAIN")):
            extracted_sql_query = possible_sql.strip()

        # Check nested JSON in tool results for SQL query
        if "content" in data and isinstance(data["content"], list):
            ...
            if inner and isinstance(inner[0], dict):
                ...
                if "custom_instructions" in j:
                    continue
                sql_in_json = j.get("sql_query") or j.get("sql") or j.get("query")
                if sql_in_json and isinstance(sql_in_json, str) and sql_in_json.strip().upper().startswith(("SELECT", "WITH", "SHOW", "EXPLAIN")):
                    extracted_sql_query = sql_in_json.strip()
    ...
    # Add extracted SQL query block if available
    if extracted_sql_query:
        blocks.append({
            "type": "sql_query",
            "query": extracted_sql_query
        })
